// --- Authentication Gate ---
// Check if user is authenticated, else redirect to login.html
// Only runs if we are NOT already on the login page (to avoid loop) and not inside an iframe (optional)
(() => {
    const isLoginPage = window.location.href.includes('login.html');
    // Check sessionStorage flag (clears when tab is closed)
    const isAuthenticated = sessionStorage.getItem('pixelFoodie_auth');
...
function populatePopularDropdown() {
    ...
    selected.forEach(recipe => {
        html += `<a href="recipe-detail?id=${recipe.id}">${recipe.title} (${recipe.rating} ★)</a>`;
    });
...
function navigateToCategory(categoryName) {
    ...
    window.location.href = `category-results?category=${encodeURIComponent(categoryName)}`;
}
